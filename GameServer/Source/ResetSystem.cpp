...
{
	this->Init();
	this->Read(gDirPath.GetNewPath(FILE_CUSTOM_RESETSYSTEM));
}
...
bool ResetSystem::ProcReset(LPOBJ lpUser, ResetType::T Type, BYTE PayType, BYTE ProcWait)
{
	...
	if (ResetGroup == NULL)
	{
		this->Log->Output("[ResetSystem] [%s] [%s] (ProcReset) Group for this user not found",
			lpUser->AccountID, lpUser->Name);
		LogAddC(2, "[ResetSystem] [%s] [%s] (ProcReset) Group for this user not found",
			lpUser->AccountID, lpUser->Name);
	...
	this->GC_MessageBox(lpUser, "Reset Master", "My regards %s!\nYou get %d reset!", lpUser->Name, lpUser->iResetCount+1);
